...
### We can store anything in any type and unknown type. A little difference.
...
```
### First we narrow down the type to number. If the data is number the code will run.
### If the data is not a number but string then the second code block will run. If the data is not a number not a string then the third code block will run.
